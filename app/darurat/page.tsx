...
export default function EmergencyPage() {
  ...
  const handleEmergencyVote = async () => {
    if (!nib || !selectedCandidate) return
    setLoading(true)
    setMessage("")

    try {
      const q = query(collection(db, "Data_Peserta"), where("NIB", "==", nib))
      const participantSnapshot = await getDocs(q)

      if (participantSnapshot.empty) {
        setMessage("Error: Peserta tidak ditemukan.")
        setLoading(false)
        return
      }

      const participantDoc = participantSnapshot.docs[0]
      if (participantDoc.data().StatusVoting === "sudah") {
        setMessage("Error: Peserta sudah memilih sebelumnya.")
        setLoading(false)
        return
      }
      ...
      <Card className="border-red-200 bg-red-50">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Nomor Induk Peserta (NIB)</Label>
            <Input
              value={nib}
              onChange={(e) => setNib(e.target.value)}
              placeholder="Masukkan NIB Peserta"
              className="bg-white"
            />
          </div>
          ...
          <Button
            ...
            disabled={loading || !nib || !selectedCandidate}
          >
